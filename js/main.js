...
onAuthStateChanged(auth, (user) => {
  ...
  * Userを示す二人称は、「あんた」もしくは「${$("#uname").val()}」です。 
...
`,
    ...
    async function getAITuberResponse(userComment) {
      try {
        ...
        // conversation historyにユーザーからのメッセージを保存
        conversationHistory.push({ role: "user", content: userComment });
        console.log(conversationHistory, "conversationHistor user");

        // もしconversationHistory内のオブジェクトが5つ(system,user*2,assistant*2)よりおおきければ削除。
        if (conversationHistory.length > 5) {
          conversationHistory.splice(1, 1); //2番めの配列を削除。戦闘はsystemなので消すとロール忘れる
        }
        console.log(conversationHistory, "conversationHistory 5以上削除1回目");
        ...
        const openAiParams = {
          ...
          body: JSON.stringify({
            model: "gpt-3.5-turbo",
            max_tokens: 2000, //ロールプレイの指示が大きいのと会話を覚えるのでトークンを大きめに設定
        ...
        console.log(
          conversationHistory,
          "conversationHistory アシスタント返答入"
        );
        // もしconversationHistory内のオブジェクトが5つ(system,user*2,assistant*2)よりおおきければ削除。
        if (conversationHistory.length > 5) {
          conversationHistory.splice(1, 1);
        }
        console.log(conversationHistory, "conversationHistory 5以上削除2回目");
